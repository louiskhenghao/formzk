...
- [Without declaration merging](#without-declaration-merging)
- [Reference](#reference)
...
> NOTE: If your TypeScript configuration isn't picking up the typings by default, you might have to adjust the `include` or `types` fields in your `tsconfig.json`.
...
> Since `@formzk/core` is headless, the layout and styling are left to the developer's discretion — this package focuses on form validation, input field rendering, form submission, and other form-related functionality. Developers can build on top of this package to further extend its features and customize it to fit their needs.
...
- `ComponentPropsOf<K>` resolves to `any` when `ComponentPropsMap` is empty, so `<Formzk.Input component="MyInput" props={...} />` does not collapse to `never`.
- `ComponentName` resolves to `string` in the unaugmented case, so `getComponent('MyInput')` / `isRegistered('MyInput')` accept any string.
...
> Refer `Formzk.Provider` documentation [here](https://github.com/louiskhenghao/formzk/blob/main/libs/core/src/core/Config/README.md)
...
> Refer `Formzk.Form` documentation [here](https://github.com/louiskhenghao/formzk/blob/main/libs/core/src/core/Form/README.md)
...
> Refer `Formzk.Input` documentation [here](https://github.com/louiskhenghao/formzk/blob/main/libs/core/src/core/FormInput/README.md)

### Formzk.Submit

The `Submit` component renders a button that triggers form submission.

> Refer `Formzk.Submit` documentation [here](https://github.com/louiskhenghao/formzk/blob/main/libs/core/src/core/FormSubmit/README.md)

### Formzk.Reset

The `Reset` component renders a button that resets the form to its default values.

> Refer `Formzk.Reset` documentation [here](https://github.com/louiskhenghao/formzk/blob/main/libs/core/src/core/FormReset/README.md)

### Formzk.Errors

The `Errors` component displays a list of form validation errors. It provides a customizable way to render error messages.

> Refer `Formzk.Errors` documentation [here](https://github.com/louiskhenghao/formzk/blob/main/libs/core/src/core/FormErrors/README.md)

---

# Quick Implementation

For those who prefer to use Material-UI, we've created a companion package called [`@formzk/mui`](https://github.com/louiskhenghao/formzk/tree/main/libs/mui#readme). This package provides pre-configured Material-UI components that seamlessly integrate with `@formzk/core`, making it quick and easy to implement forms with consistency.

---

# Extending with Other UI Libraries

If you would like to implement support for other UI libraries (Tamagui, Mantine, Chakra, etc.), we encourage you to open a PR. See [`CONTRIBUTING.md`](https://github.com/louiskhenghao/formzk/blob/main/CONTRIBUTING.md#adding-a-new-adapter-package) for the checklist on how to add a new adapter package. Your efforts will help expand the flexibility and usability of `@formzk` for everyone.
