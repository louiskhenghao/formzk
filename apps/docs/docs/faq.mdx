...
Yes — use [`@formzk/tamagui`](./tamagui/overview). It builds on Tamagui primitives, so the same form declaration renders on web (react-native-web) and native React Native. `@formzk/core` itself is renderer-agnostic.
